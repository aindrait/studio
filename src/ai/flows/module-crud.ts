...
import { z } from 'zod';
import { Module, Category, Version, AdminUser, AppSettings } from '@/lib/types';
...
// Settings Flow
export const getAppSettingsFlow = ai.defineFlow({
    name: 'getAppSettingsFlow',
    inputSchema: z.void(),
    outputSchema: AppSettingsSchema.optional(),
}, async () => {
    const db = await readDb();
    return db.settings;
});

export const updateAppSettingsFlow = ai.defineFlow({
    name: 'updateAppSettingsFlow',
    inputSchema: AppSettingsSchema,
...
export async function getAppSettings(): Promise<AppSettings | undefined> {
    return await getAppSettingsFlow();
}

export async function updateAppSettings(settings: AppSettings): Promise<AppSettings> {
    return await updateAppSettingsFlow(settings);
}
